...
mod tests {
    ...
    async fn test_connection_checkout_and_checkin() {
        ...
        let pool = ConnectionPoolV2::new(addr, PoolConfig::default());
    ...
    async fn test_max_idle_connections() {
        ...
        let pool = ConnectionPoolV2::new(addr, config);
    ...
    async fn test_concurrent_pool_access() {
        let (addr, _handle) = start_mock_server().await;
        let pool = Arc::new(ConnectionPoolV2::new(addr, PoolConfig::default()));
