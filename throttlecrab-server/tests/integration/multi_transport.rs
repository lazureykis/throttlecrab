...
use super::workload::{WorkloadConfig, WorkloadStats, WorkloadGenerator};
use super::transport_tests::{Transport, test_http_transport, test_grpc_transport, test_msgpack_transport, test_native_transport};
...
pub async fn run_multi_transport_test(workload_config: WorkloadConfig) -> Result<()> {
    ...
    cmd.arg("run")
        ...
        .arg("--")
        .arg("--http")
        .arg("--http-port").arg("28080")
        .arg("--grpc")
        .arg("--grpc-port").arg("28070")
        .arg("--msgpack")
        .arg("--msgpack-port").arg("28071")
        .arg("--native")
        .arg("--native-port").arg("28072")
        .arg("--store").arg("adaptive")
        .arg("--store-capacity").arg("200000")
        .arg("--buffer-size").arg("200000")
    ...
    for (transport, port) in transports {
        ...
        tasks.spawn(async move {
            println!("Starting {} workload on port {}", transport.flag_name(), port);
            ...
            stats.total_requests.fetch_add(summary.total_requests, std::sync::atomic::Ordering::Relaxed);
            stats.successful_requests.fetch_add(summary.successful_requests, std::sync::atomic::Ordering::Relaxed);
            stats.failed_requests.fetch_add(summary.failed_requests, std::sync::atomic::Ordering::Relaxed);
            stats.rate_limited.fetch_add(summary.rate_limited, std::sync::atomic::Ordering::Relaxed);
    ...
    println!("\nServer Performance:");
    println!("  Total throughput: {:.2} requests/sec across all transports", total_rps);
    println!("  Average per transport: {:.2} requests/sec", total_rps / 4.0);
...
pub async fn run_transport_isolation_test() -> Result<()> {
    ...
    cmd.arg("run")
        ...
        .arg("throttlecrab")
        .arg("--")
        .arg("--http").arg("--http-port").arg("38080")
        .arg("--grpc").arg("--grpc-port").arg("38070")
        .arg("--msgpack").arg("--msgpack-port").arg("38071")
        .arg("--native").arg("--native-port").arg("38072")
        .arg("--store").arg("periodic")
    ...
    println!("Sending {} requests with key '{}' across all transports", total_requests, test_key);
    ...
    println!("  Rate limited: {} ({:.2}%)", limited_count, 
