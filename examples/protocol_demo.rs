...
fn test_msgpack(port: u16, protocol_name: &str) -> std::io::Result<()> {
    println!("\n{} Protocol Test (port {})", protocol_name, port);
    println!("{}", "-".repeat(40));
    ...
    println!("Response: allowed={}, remaining={}", response.allowed, response.remaining);
    ...
    println!("Throughput test: {} requests in {:?}", num_requests, duration);
    println!("Rate: {:.0} req/s", throughput);
