...
fn benchmark_core_rate_limiter(c: &mut Criterion) {
    ...
    group.bench_function("single_key_denied", |b| {
        ...
        for _ in 0..10 {
            limiter.rate_limit(key, 5, 10, 60, 1, SystemTime::now()).unwrap();
        }
...
criterion_group!(benches, benchmark_core_rate_limiter, benchmark_memory_store_growth, benchmark_store_comparison);
